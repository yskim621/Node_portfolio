...
router.post('/save', isUser, upload.single('upfile'), joi('bookSave'), async (req, res, next) => {
	try {
		if(req.banExt) {
			...
			values = [bookName, writer, content, req.user.id]
			connect = await pool.getConnection()
...
router.get('/remove/:id', isUser, async (req, res, next) => {
	try {
		...
		sql = `DELETE FROM books WHERE id=${req.params.id} AND uid=${req.user.id}`
		connect = await pool.getConnection()
		await connect.query(sql)
		connect.release()
...
router.get('/chg/:id', isUser, async (req, res, next) => {
	try {
		...
		WHERE books.id=${req.params.id} AND books.uid=${req.user.id}`
		connect = await pool.getConnection()
		const [[rs]] = await connect.query(sql)
		connect.release()
		if(rs.savename && isImg(rs.savename)) {
...
router.post('/update', isUser, upload.single('upfile'), joi('bookUpdate'), async (req, res, next) => {
	try {
		if(req.banExt) {
			...
			values = [bookName, writer, content, id, req.user.id]
			connect = await pool.getConnection()
